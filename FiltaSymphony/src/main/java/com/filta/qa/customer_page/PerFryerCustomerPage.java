...
//		Mobile.sendKeys((int) PerFryerData.getCellDataInt(1, 18) + "");
		...
		Global_Obj.select(Service_Frequency).selectByVisibleText(PerFryerData.getCellData(1, 25));
	}
	...
	public void pricing()
	{
		...
		if (FiltaFry.isSelected() == false)
		{
			FiltaFry.click();
			lodar();
			Global_Obj.select(Pricing_Model).selectByVisibleText("Per Fryer");
			ActualPerFryerInformativeMessage = Global_Obj.alert("accept", Driver);
			lodar();
